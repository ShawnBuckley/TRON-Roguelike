...
class AxisAligned_Rectangle2
{
  ...
  public:
	...
	{
		return Vector2<t>(origin_.x + width_/2, origin_.y + height_/2);
	};
	...
	static std::vector<AxisAligned_Rectangle2<t> > Construct(std::vector<Vector2<t> > _set)
	{
		...
		if(_set.size() == 2)
		{
			...
			{
				if(std::fabs(_set[0].x) <= std::fabs(_set[1].x) && std::fabs(_set[0].y) <= std::fabs(_set[1].y))
				{
		...
		else if(_set.size())
		{
			...
			{
				if(std::fabs(_set[i].x) <= std::fabs(origin.x) && std::fabs(_set[i].y) <= std::fabs(origin.y))
					origin = _set[i];

				if(std::fabs(_set[i].x) >= std::fabs(opposite.x) && std::fabs(_set[i].y) >= std::fabs(opposite.y))
					opposite = _set[i];
			}
